...
class XTemplate(Template):
    ...
    def get_default_loss_ids(self, language:str) -> List[int]:
        ...
        '''
        return [1 if 'mask' in d else 0 for d in self.text[language]]

    def get_default_shortenable_ids(self, language:str) -> List[int]:
    ...
    def _check_template_format(self, ):
        ...
        """
        mask_num = 0
        for language, text in self.text.items():
            for i, d in enumerate(text):
                if 'mask' in d:
                    mask_num += 1

        if mask_num != len(self.text):
    ...
    def wrap_one_example(self,
        ...
        if isinstance(example, InputExample):
            ...
            for inputflag_name in self.registered_inputflag_names:
                ...
                v = None
                if inputflag_name != "shortenable_ids" and inputflag_name != "loss_ids" and hasattr(self, inputflag_name) and getattr(self, inputflag_name) is not None:
                    v = getattr(self, inputflag_name)
                elif hasattr(self, "get_default_"+inputflag_name):
                    v = getattr(self, "get_default_"+inputflag_name)(example.meta["language"])
...
class XManualTemplate(XTemplate):
    """
    Args:
    ...
    def on_text_set(self):
        ...
        self.text = {
            language:self.parse_text(text)
            for language, text in self.text.items()
        }



class XPrefixTuningTemplate(PrefixTuningTemplate):
    """
    Args:
        tokenizer (:obj:`PreTrainedTokenizer`): A tokenizer to appoint the vocabulary and the tokenization strategy.
        text (:obj:`Optional[List[str]]`, optional): manual template format. Defaults to None.
