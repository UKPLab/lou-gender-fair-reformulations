...
Namely, we use six strategies `Doppelnennung`, `GenderStern`, `GenderDoppelpunkt`, `GenderGap`, `Neutral`, and `De-e`.
...
You find in the `tasks` folder one sub-folder for each task, like `tasks/x-stance-de`. 
This sub-folder contains the splits for training, development, and testing. 
Along with the `Lou` part of DeTox, we provide to corresponding split files to run experiments and you can just put them into the corresponding sub-folder, like `tasks/detox-sentiment`.

<details>
<summary>
<strong>Open for more details about these split file.</strong>
</summary>
...
This repository use *wandb* for performance logging and saving predictions.
...
Using `run_fine_tuning_tasks.py` you can run the full fine-tuning experiments for a language model (`--model_name`) for a set of tasks.
All result will be reported to *wandb*.
<details>
<summary>
<strong>List of optional parameters.</strong>
</summary>
...
For the ICL experiments, you can either use OpenAI models or any library replicating an OpenAI endpoint, like [vLLM](https://github.com/vllm-project/vllm).
...
Using `run_icl_tasks.py`, you can run the full in-context learning experiments for a language model (`--model_name`) for a set of tasks.
